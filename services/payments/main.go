...
func init() {
	required := []string{"STRIPE_SECRET_KEY", "SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"}
	for _, key := range required {
		if os.Getenv(key) == "" {
			log.Fatalf("❌ %s no configurado en las variables de entorno", key)
		}
	}
	if os.Getenv("STRIPE_WEBHOOK_SECRET") == "" {
		log.Println("⚠️ STRIPE_WEBHOOK_SECRET no configurado — webhook en modo debug (sin verificación de firma)")
	}
}
...
func main() {
	...
	mux.HandleFunc("POST /webhook", func(w http.ResponseWriter, r *http.Request) {
		...
		if webhookSecret != "" {
			...
			parsedEvent, err := webhook.ConstructEvent(payload, sigHeader, webhookSecret)
			if err != nil {
			...
			log.Println("⚠️ Webhook sin verificación de firma (modo debug)")
